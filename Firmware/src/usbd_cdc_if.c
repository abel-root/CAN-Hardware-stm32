...
void cdc_process(void)
{
    ...
    for (uint32_t i = 0; i < rxbuf.msglen[rxbuf.tail]; i++)
    {
       ...
       {
         // Re-enable interrupts before calling command parser so USB TX
         // (which relies on interrupts) can proceed inside slcan_parse_str.
         system_irq_enable();
         int8_t result = slcan_parse_str(slcan_str, slcan_str_index);
         // Optionally signal success/failure (commented out)
         //if(result == 0) CDC_Transmit_FS("\n", 1);
         //else CDC_Transmit_FS("\a", 1);
         system_irq_disable();
