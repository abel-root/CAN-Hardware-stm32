...
static bool sw_filter_enabled = false;
static uint32_t sw_filter_ids[MAX_FILTER_IDS];
static uint8_t sw_filter_count = 0;
// Per-filter last seen data
static uint8_t sw_filter_last_data[MAX_FILTER_IDS][8];
static uint8_t sw_filter_last_dlc[MAX_FILTER_IDS];
...
void can_set_filters(uint32_t *ids, uint8_t count)
{
    if (count > MAX_FILTER_IDS)
        count = MAX_FILTER_IDS;

    for (uint8_t i = 0; i < count; i++)
        sw_filter_ids[i] = ids[i];

    sw_filter_count = count;
    sw_filter_enabled = true;
    // initialize last seen storage
    for (uint8_t i = 0; i < sw_filter_count; i++) {
        sw_filter_last_dlc[i] = 0;
    }
}

void can_disable_filter(void)
{
    sw_filter_enabled = false;
    sw_filter_count = 0;
    // clear last seen
    for (uint8_t i = 0; i < MAX_FILTER_IDS; i++) sw_filter_last_dlc[i] = 0;
}
...
uint32_t can_rx(CAN_RxHeaderTypeDef *rx_msg_header, uint8_t* rx_msg_data)
{
    ...
    // Appliquer le filtrage logiciel multi-ID
    if (sw_filter_enabled) {
        bool match = false;
        for (uint8_t i = 0; i < sw_filter_count; i++) {
            uint32_t rxid = (rx_msg_header->IDE == CAN_ID_EXT) ? rx_msg_header->ExtId : rx_msg_header->StdId;
            if (rxid == sw_filter_ids[i]) {
                match = true;
                // store last seen data
                uint8_t dlc = rx_msg_header->DLC;
                if (dlc > 8) dlc = 8;
                for (uint8_t j = 0; j < dlc; j++) sw_filter_last_data[i][j] = rx_msg_data[j];
                sw_filter_last_dlc[i] = dlc;
                break;
            }
        }
        if (!match)
            return HAL_ERROR;  // Ignorer la trame
    }
...
uint8_t can_get_last_data(uint32_t id, uint8_t *out_data, uint8_t *out_dlc)
{
    if (!sw_filter_enabled) return 0;
    for (uint8_t i = 0; i < sw_filter_count; i++) {
        if (sw_filter_ids[i] == id) {
            uint8_t dlc = sw_filter_last_dlc[i];
            if (dlc == 0) return 0;
            for (uint8_t j = 0; j < dlc; j++) out_data[j] = sw_filter_last_data[i][j];
            *out_dlc = dlc;
            return 1;
        }
    }
...
uint8_t can_is_on_bus(void)
{
    return (bus_state == ON_BUS) ? 1 : 0;
}
